...
const BlogForm = (props) => {
  ...
  const handleSubmitBlog = (e) => {
    ...
    if (isEdited !== true) {
      ...
      axios
        .patch(
          "http://localhost:4000/blog/" + props.location.state.detail._id,
          { title: blog.title, body: blog.body, tags: blog.tags },
          {
            headers: {
              Authorization: localStorage.getItem("token"),
  ...
  const handleKeyPress = (e) => {
    ...
    if (e.charCode == 13) {
      ...
      document
        ...
        .insertAdjacentHTML(
          "beforeend",
          `<div class="bagge badge-pill badge-info span-tag flex-container-row"><span style="margin:3px" class="bagge badge-pill badge-info span-tag">${tag}</span></div> `
        );
            ...
            <form onSubmit={handleSubmitBlog}>
              ...
              <div className="form-group font-blogForm">
                ...
                <div
                  ...
                  style={{ marginTop: 10 + "px", flexWrap: "wrap" }}
                >
                  {isEdited === true
                    ? blogTags.map((tag) => (
                        <BlogInForm key={tag} {...props} tag={tag} />
                      ))
                    : null}
                </div>
              </div>
              <div
                className="btn-blogForm-container"
                style={{ marginLeft: 60 + "px", marginTop: 20 + "px" }}
              >
                <button
                  type="submit"
                  className="btn btn-info font-blogForm btn-margin-BlogForm"
                  onClick={handleSubmitBlog}
                >
                  {" "}
                  {isEdited === true ? "Edit" : "Add"}
                </button>
              </div>
            </form>
          </div>
